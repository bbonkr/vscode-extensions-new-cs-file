...
describe('findProjectFile', () => {
  ...
  describe('Correct', () => {
    it('should find csproj file #1', () => {
      ...
      assert.equal(projFilePath, path.join(process.cwd(), `${workspaceBase}/${projectFileName}`));
    });

    it('should find csproj file #2', () => {
      const workspaceBase = 'forTest/Type2';
      const currentPath = path.join(process.cwd(), `${workspaceBase}/src/Sample/Models`);
      const workspacePath = path.join(process.cwd(), workspaceBase);
      ...
      assert.equal(
        projFilePath,
        path.join(process.cwd(), `${workspaceBase}/src/Sample/${projectFileName}`),
      );
    });
